"""Plot PCG experiment results comparing GPU vs CPU solve times."""
...
def load_all_results(results_dir: str = "./experiment_results") -> pd.DataFrame:
    """Load all PCG CSV files from the results directory.
    ...
    # Look for PCG-specific CSV files
    csv_files = list(results_path.glob("pcg_*.csv"))

    # Exclude the combined results file
    csv_files = [f for f in csv_files if f.name != "pcg_all_results.csv"]

    if not csv_files:
        raise ValueError(f"No PCG CSV files found in {results_dir}")
...
def create_comparison_plot(
    df: pd.DataFrame,
    output_path: str = "./experiment_results/pcg_solve_time_comparison.pdf",
):
    """Create bar chart comparing GPU vs CPU solve times for PCG.
    ...
    # Group by dataset and device type, use total_time (construction + solve)
    grouped = (
    ...
    ax.set_title(r"\textbf{PCG Solver: GPU vs CPU Performance Comparison}", fontsize=14)
    ax.set_xticks(x)
    ...
    # Show time breakdown
    print("\n" + "=" * 60)
    print("Time Breakdown (Construction vs Solve)")
    print("=" * 60)
    time_breakdown = (
        converged_df.groupby(["dataset", "device_type"])
        .agg({"construction_time": "mean", "solve_time": "mean", "total_time": "mean"})
        .reset_index()
    )

    for _, row in time_breakdown.iterrows():
        print(f"\n{row['dataset']} on {row['device_type']}:")
        print(
            f"  Construction: {row['construction_time']:.2f}s ({100 * row['construction_time'] / row['total_time']:.1f}%)"
        )
        print(
            f"  Solve:        {row['solve_time']:.2f}s ({100 * row['solve_time'] / row['total_time']:.1f}%)"
        )
...
def main():
    ...
    if not os.path.exists(results_dir):
        ...
        print("Please run experiments first using ./run_pcg_all_experiments.sh")
        return
