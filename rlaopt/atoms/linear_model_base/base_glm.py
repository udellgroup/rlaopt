...
from rlaopt.atoms.linear_model_base.base_regressor import _get_central_tendency, _get_regression_score
...
class BaseGLM(_BaseLinearModel, ABC):
    ...
    def forward(self):
        ...
        return BaseGLM._get_loss(beta_tensor, 
                                 intercept_tensor,
                                 self._loss_fn,
                                 self.dataloader,
    ...
    def score(self, beta_value = None, X = None, y = None):
        ...
        y_null = _get_central_tendency(
            y_true, type="mean"
        )
        model_deviance, null_deviance = self.deviance_fn(y_model, y_true), self.deviance_fn(y_null, y_true)
