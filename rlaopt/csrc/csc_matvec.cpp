...
torch::Tensor csc_matvec_cpu_impl(const torch::Tensor& sparse_tensor, const torch::Tensor& dense_vector) {
...
torch::Tensor csc_matvec_cpu(const torch::Tensor& sparse_tensor, const torch::Tensor& dense_vector) {
...
TORCH_LIBRARY_IMPL(rlaopt, SparseCsrCPU, m) {
    m.impl("csc_matvec", &csc_matvec_cpu);
}
