...
__global__ void csc_matvec_kernel(
...
torch::Tensor csc_matvec_cuda(const torch::Tensor& sparse_tensor, const torch::Tensor& dense_vector) {
    ...
    TORCH_CHECK(sparse_tensor.device().type() == at::DeviceType::CUDA, "Input tensor must be on CUDA");
    TORCH_CHECK(dense_vector.device().type() == at::DeviceType::CUDA, "dense_vector must be on CUDA");

    TORCH_CHECK(sparse_tensor.dtype() == dense_vector.dtype(), "sparse_tensor and dense_vector must have the same dtype");
    ...
    TORCH_CHECK(num_cols == dense_vector.size(0), "Number of columns in sparse tensor must match dense vector size");
...
TORCH_LIBRARY_IMPL(rlaopt, SparseCsrCUDA, m) {
    m.impl("csc_matvec", &csc_matvec_cuda);
}
