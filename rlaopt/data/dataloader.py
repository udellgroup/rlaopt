...
from rlaopt.data import BatchedDataset, Dataset


class DataLoader(torch.utils.data.DataLoader):
    """Extended PyTorch DataLoader with support for custom Dataset types and lazy label access.

    This DataLoader extends torch.utils.data.DataLoader to work specifically with Dataset
    and BatchedDataset types, providing additional functionality for accessing training
    labels efficiently based on the dataset type.
    ...
    Attributes:
        y: Property that returns all training labels from the dataset. For Dataset instances,
           labels are retrieved directly from memory. For BatchedDataset instances, labels
           are collected by iterating through all batches.
...
def get_training_labels(loader: DataLoader, in_memory: bool):
    ...
    Args:
        ...
        in_memory: If True, retrieves labels directly from dataset.y (for Dataset).
                   If False, iterates through all batches to collect labels (for BatchedDataset).
