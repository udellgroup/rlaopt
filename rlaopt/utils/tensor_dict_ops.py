...
def has_compatible_ordered_names(x: TensorDict, y: TensorDict, check_lengths: bool = True) -> bool:
    """
    ...
    if check_lengths and not has_compatible_lengths(x, y):
        print(f"Incompatible lengths: {len(x)} vs {len(y)}. "
              "Names cannot be compatible as x and y have different number of leaves.")
        return False
    
    false_indices = [(i, x_key, y_key)
...
def has_compatible_ordered_shapes(x: TensorDict, y: TensorDict, check_lengths: bool = True) -> bool:
    """
    ...
    false_indices = [(i, x_leaf.shape, y_leaf.shape)
                     for i, (x_leaf, y_leaf) in enumerate(zip(x.values(), y.values()))
                     if x_leaf.shape != y_leaf.shape]
    
    if false_indices:
        print(f"Incompatible shapes at leaves (index, x_shape, y_shape): {false_indices}")
        return False
    return True
...
def is_compatible(x: TensorDict, y: TensorDict) -> bool:
    ...
    """
    return (has_compatible_ordered_names(x, y) and
            has_compatible_ordered_shapes(x, y, check_lengths=False))
