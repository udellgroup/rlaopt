...
def _generate_test_data(dataloader: DataLoader, device: torch.device) -> tuple[torch.Tensor]:
    n, p = dataloader.dataset.num_samples, dataloader.dataset.feature_dimension
    n_test = n // 5
    X_test, y_test = torch.randn((n_test, p), device=device), torch.randn(n_test, device=device)
