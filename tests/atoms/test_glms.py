...
from rlaopt.data import DataLoader, Dataset, BatchedDataset
...
class BatchedGLMDataset(BatchedDataset):
    ...
    def __init__(self, n_samples, n_features, seed=42, device=torch.device("cpu")):
        ...
        self._true_beta = torch.randn(n_features, device=device).clamp_(min=-1.0, max=1.0)
        
        # Generate all data upfront for consistency
        self._X = torch.randn(n_samples, n_features, device=device)
        self._y = torch.exp(self._X @ self._true_beta) + torch.rand(n_samples, device=device)
