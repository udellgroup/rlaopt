...
class TestHalfspace:
    ...
    def test_init_with_halfspace_constraint(self, vector_var, simple_halfspace_data):
        ...
        assert halfspace.get_buffer("C") is not None
        assert halfspace.get_buffer("upper") is not None
        assert halfspace.get_buffer("A") is None
        assert halfspace.get_buffer("b") is None
    ...
    def test_init_stores_constraint_correctly(
        ...
        assert torch.equal(halfspace.get_buffer("C"), coordinate_halfspace_data["c"])
        assert halfspace.get_buffer("upper") == coordinate_halfspace_data["upper"]
    ...
    def test_forward_returns_zero_when_satisfied(self, small_vector_var):
        ...
        small_vector_var.value = torch.tensor([1.0, 1.0, 1.0])  # Sum = 3 <= 5
    ...
    def test_forward_returns_zero_at_boundary(self, small_vector_var):
        ...
        small_vector_var.value = torch.tensor([1.0, 1.0, 1.0])  # Sum = 3 = upper
        result = halfspace.forward()
        assert torch.allclose(result, torch.tensor(0.0))
    ...
    def test_forward_returns_inf_when_violated(self, small_vector_var):
        ...
        small_vector_var.value = torch.tensor([1.0, 1.0, 1.0])  # Sum = 3 > 2
        result = halfspace.forward()
    ...
    def test_prox_returns_identity_for_feasible_point(
        ...
        halfspace = Halfspace(vector_var, **simple_halfspace_data)
        location = TensorDict(
            {vector_var.name: torch.tensor([0.5, 0.5, 0.5, 0.5, 0.5])}
        )  # Sum = 2.5 <= 5
    ...
    def test_prox_projects_infeasible_point(self, vector_var, simple_halfspace_data):
        ...
        halfspace = Halfspace(vector_var, **simple_halfspace_data)
        location = TensorDict(
            {vector_var.name: torch.ones(5) * 2}
        )  # Sum = 10 > 5, violates constraint

        result = halfspace.prox(location, prox_scaling=1.0)

        # Check result satisfies constraint
        result_tensor = result[vector_var.name]
        assert (
            torch.dot(halfspace.get_buffer("C"), result_tensor)
            <= halfspace.get_buffer("upper") + 1e-5
        )
    ...
    def test_prox_moves_perpendicular_to_boundary(self, small_vector_var):
        ...
        location_tensor = torch.tensor([5.0, 2.0, 3.0])  # x[0] = 5 > 1, violates
        location = TensorDict({small_vector_var.name: location_tensor})

        result = halfspace.prox(location, prox_scaling=1.0)

        # Only first coordinate should change (perpendicular to boundary)
        result_tensor = result[small_vector_var.name]
        assert torch.allclose(result_tensor[1:], location_tensor[1:])
        assert result_tensor[0] <= 1.0 + 1e-5
    ...
    def test_prox_at_boundary_unchanged(self, vector_var, simple_halfspace_data):
        ...
        halfspace = Halfspace(vector_var, **simple_halfspace_data)
        location = TensorDict(
            {vector_var.name: torch.ones(5)}
        )  # Sum = 5 = upper (on boundary)
    ...
    def test_prox_independent_of_scaling(self, vector_var, simple_halfspace_data):
        ...
        halfspace = Halfspace(vector_var, **simple_halfspace_data)
        location = TensorDict({vector_var.name: torch.ones(5) * 2})  # Sum = 10 > 5
    ...
    def test_prox_with_coordinate_constraint(
        ...
        halfspace = Halfspace(vector_var, **coordinate_halfspace_data)
        location_tensor = torch.tensor([5.0, 2.0, 3.0, 4.0, 1.0])  # x[0] = 5 > 1
        ...
        result_tensor = result[vector_var.name]
        assert torch.allclose(result_tensor[0], torch.tensor(1.0))
        # Others unchanged
        assert torch.allclose(result_tensor[1:], location_tensor[1:])
    ...
    def test_prox_minimizes_distance(self, vector_var, simple_halfspace_data):
        ...
        halfspace = Halfspace(vector_var, **simple_halfspace_data)
        location_tensor = torch.tensor([2.0, 2.0, 2.0, 2.0, 2.0])  # Sum = 10 > 5
        location = TensorDict({vector_var.name: location_tensor})
        ...
        distance = torch.norm(result_tensor - location_tensor)
        assert distance > 0  # Must move
        assert distance < torch.norm(location_tensor)  # But not too far
    ...
    def test_with_negative_upper_bound(self, small_vector_var):
        ...
        location = TensorDict(
            {small_vector_var.name: torch.zeros(3)}
        )  # Sum = 0 > -1, violates
    ...
    def test_with_scaled_normal_vector(self, small_vector_var):
        ...
        location = TensorDict(
            {small_vector_var.name: torch.ones(3) * 2}
        )  # c^T x = 12 > 6
