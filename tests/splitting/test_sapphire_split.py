...
from rlaopt.atoms import LinearRegression, LogisticRegression, SumSquares, L1Norm, Box
from rlaopt.data import Dataset, DataLoader
from rlaopt.expression import Variable, AddExpression
...
class TestInitialization:
    ...
    def test_invalid_input(self, log_reg_model, problem_data):
        ...
        with pytest.raises(ValueError, match="Smooth expression depends upon variables"):
            SapphireSplit(model + L1Norm(beta) + SumSquares(y))

        # Non smooth expr depends upon variable other than beta
        with pytest.raises(ValueError, match="Non-smooth regularizer depends upon variables"):
...
class TestOracles:
    ...
    def test_evaluate(self, problem_data, log_reg_model):
        ...
        + torch.linalg.norm(beta.forward()) ** 2 + torch.linalg.norm(beta.forward(), ord=1)
    ...
    def test_loss(self, problem_data, log_reg_model):
         ...
         truth = _logistic_loss(beta.forward(), X_batch, y_batch) + 0.5 * torch.linalg.norm(beta.forward()) ** 2 
         
         assert torch.allclose(output_, truth)
    
    def test_grad_oracles(
            self, problem_data, log_reg_model
    ):
        beta, dataset = problem_data
        model, _ = log_reg_model
        ...
        true_grad = _logistic_grad(beta.forward(), dataset.X, dataset.y) + beta.forward()

        assert torch.allclose(output_grad, true_grad)

        X_batch, y_batch = dataset.X[0:10], dataset.y[0:10]

        batch_grad = split.batch_grad_loss(model.variable_values, X_batch, y_batch)['beta']

        true_batch_grad = _logistic_grad(beta.forward(), X_batch, y_batch) + beta.forward()
    ...
    def test_subsampled_hvp(
        ...
        Hop = split.get_subsamp_hessian_linop(model.variable_values, X_batch, y_batch, X_batch.device)
