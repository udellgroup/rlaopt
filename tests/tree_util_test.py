...
class TestTreeUtil(TestCase):
    ...
    def test_tree_vdot_real(self):
        ...
        expected = np.vdot(self.array_A, self.array_C).real
        got = tree_util.tree_vdot_real(self.array_A, self.array_C)
        self.assertAllClose(expected, got)

        expected = sum([np.vdot(self.tree_A[i], self.tree_B[i]).real for i in range(2)])
    ...
    def test_tree_inf_norm(self):
        ...
        expected = jnp.maximum(
            jnp.max(jnp.abs(self.tree_A[0])), jnp.max(jnp.abs(self.tree_A[1]))
        )
        got = tree_util.tree_inf_norm(self.tree_A)
        self.assertAllClose(expected, got)

        tree_a_with_empty_leaves = (
            {"a": self.tree_A, "b": jnp.array([])},
            jnp.array([]),
        )
    ...
    def test_tree_conj(self):
        ...
        expected = (np.conj(self.tree_A[0]), np.conj(self.tree_A[1]))
    ...
    def test_tree_real(self):
        ...
        expected = (np.real(self.tree_A[0]), np.real(self.tree_A[1]))
    ...
    def test_tree_imag(self):
        ...
        expected = (np.imag(self.tree_A[0]), np.imag(self.tree_A[1]))
    ...
    def test_tree_where(self):
        ...
        self.assertAllClose(d[0], jnp.array([1.0, 42.0, 3.0, 42.0]))
        self.assertAllClose(d[1], jnp.array([5.0, 13.0, 13.0, 8.0]))
        ...
        b = jnp.array(42.0)
        d = tree_util.tree_where(c, a, b)
        self.assertAllClose(d[0], jnp.array([42.0, 42.0, 42.0, 42.0]))
    ...
    def test_tree_single_dtype(self, high_precision, convert_in_jax_dtype):
        ...
        dtype_conversions = {
            np.asarray(True).dtype: jnp.asarray(True).dtype,
            np.asarray(0).dtype: jnp.asarray(0).dtype,
            np.asarray(0.0).dtype: jnp.asarray(0.0).dtype,
            np.asarray(1j * 1.0).dtype: jnp.asarray(1j * 1.0).dtype,
        }
        ...
        for x in test_base_types:
            dtype_tree = tree_util.tree_single_dtype(
                x, convert_in_jax_dtype=convert_in_jax_dtype
            )
            dtype_leaf = np.asarray(x[0]).dtype
            if not high_precision and convert_in_jax_dtype:
                if dtype_leaf not in (np.asarray(True).dtype, jnp.asarray(0.0).dtype):
                    assert dtype_tree != dtype_leaf
        ...
        with pytest.raises(ValueError):
            tree_util.tree_single_dtype((1.0, 1))

        class MyInt:
            i: int = 1

        assert tree_util.tree_single_dtype(MyInt()) is None
